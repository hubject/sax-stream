...
function XmlNode(options) {
    ...
    // Emit events per tag
    const superEmit = this.emit;
    this.emit = (event, ...args) => {
        if (event === 'data') {
            const {key, value} = args[0];
            superEmit.call(this, key, value);
            return superEmit.call(this, 'data', value);
        }
        return superEmit.call(this, event, ...args);
    };
    // Dummy event handler for data needs to be
    // registered to ensure that events per tag
    // are fired as well
    this.on('data', () => null);
}
...
XmlNode.prototype.pushAll = function(callback) {
    ...
    this.records.forEach(record => self.push(record));
